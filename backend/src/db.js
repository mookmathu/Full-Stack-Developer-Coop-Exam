...
export const initDB = async () => {
  await db.query(`CREATE DATABASE IF NOT EXISTS fleet_db`);
  await db.query(`USE fleet_db`);
  ...
  if (users.length === 0) {
    ...
    await db.query(
      `INSERT INTO users (id, username, password_hash, role) VALUES (UUID(), 'admin', ?, 'ADMIN')`,
      [hash]
    );
